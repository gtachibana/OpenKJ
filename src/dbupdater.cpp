...
void DbUpdater::DiskEnumerator::readNextDiskFile()
{
    ...
    do {
        ...
        if (CurrentFile != nullptr && CurrentFile.endsWith(".cdg", Qt::CaseInsensitive)) {
            ...
            const QString disk_path_without_ext = CurrentFile.left(CurrentFile.length() - 4);
...
void DbUpdater::fixMissingFiles(QVector<DbSongRecord> &filesMissingOnDisk, QStringList &newFilesOnDisk) {
    ...
    QVector<QPair<QString, QString>> filesOnDiskByFilename;
    filesOnDiskByFilename.reserve(newFilesOnDisk.size());
    for (const auto &path : newFilesOnDisk) {
        filesOnDiskByFilename.append({QFileInfo(path).fileName(), path});
    }

    auto caseInsensitiveSort = [](const QPair<QString, QString> &a, const QString &b) -> bool {
        return QString::compare(a.first, b, Qt::CaseInsensitive) < 0;
    };
    auto caseInsensitiveSortPair = [](const QPair<QString, QString> &a, const QPair<QString, QString> &b) -> bool {
        return QString::compare(a.first, b.first, Qt::CaseInsensitive) < 0;
    };

    std::sort(filesOnDiskByFilename.begin(), filesOnDiskByFilename.end(), caseInsensitiveSortPair);
    ...
    foreach(auto missingFile, filesMissingOnDisk) {
        ...
        auto const lb = std::lower_bound(filesOnDiskByFilename.begin(), filesOnDiskByFilename.end(), filenameWithoutPath, caseInsensitiveSort);
        if (lb != filesOnDiskByFilename.end() && QString::compare(lb->first, filenameWithoutPath, Qt::CaseInsensitive) == 0) {
            query.bindValue(":newpath", lb->second);
            ...
            if (query.exec()) {
                ...
                newFilesOnDisk.removeOne(lb->second);
                matchFound = true;
            }
            else {
